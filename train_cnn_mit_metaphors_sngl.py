...
def run_cnn_model():
    ...
    for domain in domains:
  	test_acc = 0.0
        print "Getting results for domain:" + domain
        revws, labels = get_domain_revs(domain, lda, dictionri, word2idx, model)
           #print labels
        kf = KFold(n_splits=10, shuffle=True, random_state=666) 
        for train, test in kf.split(labels): 
    		x_tr, x_te, y_tr, y_te = revws[train], revws[test], labels[train], labels[test]
	        max_sent_len = np.max([len(sent) for sent in x_tr]) 
		cv_dat = x_tr, x_te, y_tr, y_te
        	print "Single domain CV:"
    		test_acc = test_acc + cnn_model(embedding_weights, cv_dat, max_sent_len)
    
    	print "Done training the model, average accuracy on CV set for domain:" + domain
    	print test_acc/10
        acc_dict[domain] = test_acc/10
