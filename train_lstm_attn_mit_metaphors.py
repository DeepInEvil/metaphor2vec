...
from keras.layers import Dense, Dropout, Embedding, Conv1D, GlobalAveragePooling1D, LSTM, Layer, TimeDistributed
...
vocab_path='/data/dchaudhu/metaphor2vec/data/vocab/vocab_mit_metaphors.npy'
word_vec_file='/data/dchaudhu/metaphor2vec/data/word_vecs/mit_metaphor/domain_vecs'
glove_word_vecs = '/data/dchaudhu/metaphor2vec/data/word_vecs/mit_metaphor/glove/glove_corpus_vecs'
...
def get_index_to_embeddings_mapping(vocab, word_vecs):
    ...
    for word in vocab.keys():
        try:
            embeddings[word] = word_vecs.word_vectors[word_vecs.dictionary[word]]
        except KeyError:
...
def lstm_model(embedding_weights, cv_dat, max_len):
    max_len = max_len
    dropout = 0.5
    ...
    merge_pooling = concatenate([glob1a, glob2a, glob3a, glob4a])
    print merge_pooling
    """ 
    lstm_layer = LSTM(128, return_sequences=True)(embedded_layer_body)
    l_dense = TimeDistributed(Dense(600))(lstm_layer)
    attention = AttLayer()(l_dense) 
    hidden_layer = Dense(1200, activation='tanh', kernel_initializer="glorot_uniform")(attention)
...
def run_cnn_model():
    ...
    for domain in domains:
    	...
    	test_acc = test_acc + lstm_model(embedding_weights, cv_dat, max_sent_len)
